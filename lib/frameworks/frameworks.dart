...
import '../screens/detail/themes/product_detail_gallery_slider_layout.dart';
import '../screens/detail/themes/web_simple_type.dart';
import '../screens/detail/widgets/product_price/product_price_widget.dart';
...
//Sync cart to website
  ...
  /// Renders the vendor admin interface. Opens native app if available, else via WebView.
  Widget renderVendorAdmin(BuildContext context, User user) {
    final typeName = ServerConfig().typeName;
    final serverConfigUrl = ServerConfig().url;

    final vendorDashboardUrl = kVendorConfig.vendorDashboardUrl.isNotEmpty
        ? kVendorConfig.vendorDashboardUrl
        : typeName == 'wcfm'
            ? (kVendorConfig.wcfm.isNotEmpty
                ? '$serverConfigUrl/${kVendorConfig.wcfm}'
                : '$serverConfigUrl/store-manager?vendor_admin=true')
            : (['listeo', 'dokan', 'woo'].contains(typeName) &&
                    kVendorConfig.dokan.isNotEmpty)
                ? '$serverConfigUrl/${kVendorConfig.dokan}'
                : '$serverConfigUrl/my-account?vendor_admin=true';

    final webView = WebView(
      vendorDashboardUrl.addWooCookieToUrl(user.cookie),
      title: S.of(context).vendorAdmin,
      script: kAdvanceConfig.webViewScript,
    );

    return kVendorConfig.disableNativeStoreManagement
        ? webView
        : Services().getVendorAdminApp(user: user, isFromMV: true) ?? webView;
  }
  ...
  Widget renderDetailScreen(
    ...
    switch (layoutType) {
      ...
      case 'gallerySlider':
        return ProductDetailGallerySliderLayout(
          product: product,
          isProductInfoLoading: isProductInfoLoading,
        );
      default:
  ...
  /// Renders the delivery interface. Opens native app if available, else via WebView.
  Widget renderDelivery(BuildContext context, User user) {
    final typeName = ServerConfig().typeName;
    final serverConfigUrl = ServerConfig().url;

    final deliveryDashboardUrl = kVendorConfig.deliveryDashboardUrl.isNotEmpty
        ? kVendorConfig.deliveryDashboardUrl
        : typeName == 'wcfm'
            ? '$serverConfigUrl/store-manager/deliveries/'
            : '$serverConfigUrl/${[
                'listeo',
                'dokan',
                'woo'
              ].contains(typeName) ? 'my-account/driver-dashboard/' : 'my-account?delivery=true'}';

    final webView = WebView(
      deliveryDashboardUrl.addWooCookieToUrl(user.cookie),
      title: S.of(context).deliveryManagement,
      script: kAdvanceConfig.webViewScript,
    );

    return kVendorConfig.disableNativeDeliveryManagement
        ? webView
        : Services().getDeliveryApp(user: user, isFromMV: true) ?? webView;
  }

  /// render addons options cart item
  ...
  Widget renderHomeSearchScreen() => ChangeNotifierProvider(
      ...
      builder: (context, _) {
        return SearchScreen(
          showQRCode: context.read<AppModel>().appConfig?.showQRCode,
        );
      });
  ...
  Widget renderDetailPrice(
    ...
    if (['variable-subscription', 'subscription'].contains(product.type)) {
      return DetailPrice(product: product, price: priceData?.price);
    }

    /// Render detailed price includes regular price and sale price and countdown timer
    return ProductPrice(
      product: product,
      priceData: priceData,
  ...
  Widget renderCODExtraFee(BuildContext context) {
    ...
    final enabled = (kPaymentConfig.smartCOD?.enabled ?? false) &&
        kCashOnDeliveryConfig
            ...
            .contains(cartModel.paymentMethod?.id);
