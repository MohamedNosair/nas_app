...
mixin DetailedBlogMixin {
  Widget renderBlogFunctionButtons(Blog blog, BuildContext context) =>
      Row(
  ...
  Widget renderBlogContentWithTextEnhancement(Blog blog) =>
      Consumer<TextStyleModel>(builder: (context, textStyleModel, child) {
        var html = parse(blog.content);
        var text = '';
        if (html.getElementById('skin_default') != null) {
          html.getElementById('skin_default')?.remove();
          text = html.outerHtml.replaceAll('<strong>راديو الناس – بث مباشر&nbsp;</strong>', "");
        }else {
          text = html.outerHtml;
        }
  ...
  Widget _renderShareButton(Blog blog) =>
      ((kBlogDetail['showSharing'] ?? true) && dynamicLinkConfig.allowShareLink)
